...
public static class misc
{
    ...
    public static string ReadNullTerminatedString(this sio.Stream str, int maxBytesLen, ste enc)
    {
        ...
        else
        {
            ...
            return text.Streams.reader.ReadStringUsingTable(buf, 0, ++maxBytesLen);
    ...
    public static string ReadNullTerminatedString(this sio.Stream str, ste enc)
    {
        const int bufInc = 256;
        byte[] buf = new byte[bufInc];
        int bufCnt = 0;
        byte b;
        ...
        while (b != 0)
        {
            ...
            if (++bufCnt % bufInc == 0)
                Array.Resize<byte>(ref buf, buf.Length + bufInc);

            b = (byte)str.ReadByte();
        }

        return enc.GetString(buf, 0, bufCnt);
    }
